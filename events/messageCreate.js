...
module.exports = async (message) => {
  ...
  if (userIsBlocked(message.author))
    return void await message.reply({content: '<:no:803069123918823454> You are blocked from using commands'});
  ...
  if (message.guild) {
    ...
    if (!checkPermissions(message.member, message.channel, command.permissions))
      return void await message.reply({content: '<:no:803069123918823454> You do not have permission to use this command.'});

    if (!checkPermissions(message.member, message.channel, command.clientPermissions))
      return void await message.reply({content: '<:no:803069123918823454> looks like **I** don\'t have permission do run that command. Ask a server mod for help and try again later.'});
...
async function handleLevels(message) {
  ...
  let messages;
  if (messagefetch === 25 + 625 * levelfetch + Math.floor(levelfetch / 3))
