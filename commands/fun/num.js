...
module.exports = {
	...
	async execute(message, args) {
		...
		if (!fs.existsSync(`./1-${args}.txt`)) {
			// file exists
			return await message.channel.send('You are using an unsupported number. The supported numbers are 100, 1000, 10000, 100000, 1000000.  More coming soon!');
		}
		...
		message.reply({ content: `${message.author}, here is the numbers 1-${args}`, files: [attachment] });
