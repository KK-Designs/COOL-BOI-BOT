...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args[0]?.toLowerCase() === "reroll") {
      ...
      if (!messageID)
        return sendError('Please provide a valid message id', message.channel);
    ...
    if (args[0]?.toLowerCase() === "edit") {
      ...
      if (!args[2])
        return sendError('Please provide a valid time. like `5m`', message.channel);
    ...
    switch (subCmd) {
      case "create": {
        ...
        if (!Number.isInteger(time) || time <= 0)
          return interaction.reply({content: "Invalid time provided", ephemeral: true});
      ...
      case "edit": {
        ...
        try {
          ...
          if (typeof e !== "string" || !e.startsWith("No giveaway found with message Id "))
            throw e;
