	...
	execute(message, args, client) {
		...
		if (args.length) {
			...
			if (args[0].toLowerCase() === 'edit') {
				...
				if (!args[2]) {
					return sendError(
						'Please provide a valid time. like `5m`',
						message.channel,
					);
				}
				if (!args.slice(3).join(' ')) {
					return sendError(
						'Please provide a valid prize like `1 month nitro`',
						message.channel,
					);
				}
				client.giveawaysManager
					...
					.then(() => {
						...
						message.channel.send({
							content:
								'<:check:807305471282249738> Success! Giveaway will updated soon.',
						});
					})
					.catch((err) => {
						sendError(
			...
			if (args[0].toLowerCase() === 'delete') {
				...
				client.giveawaysManager
					...
					.then(() => {
						message.channel.send({
							content: '<:check:807305471282249738> Success! Giveaway deleted!',
						});
					})
					.catch((err) => {
						message.channel.send({
