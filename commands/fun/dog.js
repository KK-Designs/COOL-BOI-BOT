...
module.exports = {
	...
	async execute(message, args) {
		if (!args[0]) {
			const res = await fetch('https://api.thedogapi.com/v1/images/search').then(res => res.json());
			...
			const res = await fetch(`https://api.thedogapi.com/v1/breeds/search?q=${encodeURIComponent(args.join(' '))}`).then(res => res.json());
			if (!res.length) {
	...
	async executeSlash(interaction) {
		...
		if (!breed) {
			...
			const res = await fetch(`https://api.thedogapi.com/v1/breeds/search?q=${encodeURIComponent(breed)}`).then(res => res.json());
			if (!res.length) {
