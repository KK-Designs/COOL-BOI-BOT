	...
	async execute(message, args) {
		...
		setTimeout(async() => {
			...
			if (user) {
               ...
               */
					message.guild.members.kick(user.id, { reason: reason })
						.then(() => {
							if (reason === '') {
								reason = 'No reason provided';
							}
						...
						.catch(err => {
							...
							return sendError('I was unable to kick that user. Check if I have the permision `KICK_MEMBERS`. If not that make my my role is higher than the member you are tying to kick.', message.channel);
							// Log the error
							console.error(err);
						});
				}
					...
					return message.channel.send({ content: 'That user isn\'t in this guild!', reply: { messageReference: message.id } });
				}
				// Otherwise, if no user was mentioned
			}
			else {
				return message.channel.send({ content: 'You didn\'t mention the user to kick!', reply: { messageReference: message.id } });
			}
		}, 1500);
	},
};