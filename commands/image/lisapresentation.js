module.exports = {
	...
	clientPermissons: ['EMBED_LINKS', 'ATTACH_FILES'],
	  async execute(message, args) {
		...
		if (!text) return message.channel.send({ content: 'Please provide text for me to make a lisa presentation', reply: { messageReference: message.id } });
		if (text.length > 300) return message.channel.send({ content: 'I can only make a lisa presentation out of 300 characters', reply: { messageReference: message.id } });
