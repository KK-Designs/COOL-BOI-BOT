module.exports = {
	...
	execute(message, args) {
		...
		if (!args.length) {
			...
			data.push(
				`\nYou can use \`${
					guildPrefix == null ? '!' : guildPrefix
				}help [command name]\` to get info on a specific command. Use \`${
					guildPrefix == null ? '!' : guildPrefix
				}help [category name]\` to get specific category commands. Make sure to use \`${
					guildPrefix == null ? '!' : guildPrefix
				}\` or @mention me before each command!`,
			);
			...
			return message.channel
				...
				.then((m) => {
					...
					collector.on('collect', (i) => {
						if (i.customId === 'categorySelectMenu') {
							if (i.user.id === message.author.id) {
								const category = commands.filter(
									(command) => command.category === i.values[0],
								);
								const cmds = [];
								const categorycmd = category.map(
								...
								cmds.push(
									`\n\nYou can use \`${
										guildPrefix == null ? '!' : guildPrefix
									}help [command name]\` to get info on a specific command. Make sure to use \`${
										guildPrefix == null ? '!' : guildPrefix
									}\` or @mention me before each command!`,
								);
								...
								i.reply({
									content:
										'<:X_:807305490160943104> You are not allowed to use the select menu on this command',
									ephemeral: true,
								});
							}
						}
					...
					collector.on('end', (collected) => {
						const categoryMenu1 = new MessageActionRow().addComponents(
							new MessageSelectMenu()
								.setCustomId('categorySelectMenu')
								.setPlaceholder('Click here to select a category')
								.addOptions([
									...
									},
									{
										label: 'General',
										description:
											'All of the commands in the "general" category',
										value: 'general',
									},
									{
										label: 'Games',
									...
									},
									{
										label: 'Moderation',
										description:
											'All of the commands in the "moderation" category',
										value: 'moderation',
									},
									{
										label: 'Music',
		...
		if (!command && !categorynames.includes(name)) {
			...
			let error = `that\'s not a valid command, did you mean \`${result.join(
				' or ',
			)}\`?`;
			if (!result.length) {
		...
		if (command.permissions) {
			perms.push(
				`\`${command.permissions}\`, \`SEND_MESSAGES\`, \`VIEW_CHANNEL\``,
			);
		}
		...
		const embed = new MessageEmbed()
			...
			.addFields(
				...
				{
					name: '**Usage**',
					value: `${guildPrefix == null ? '!' : guildPrefix}${command.name} ${
						command.usage
					}`,
					inline: true,
				},
				...
				{ name: '**Permissions**', value: `${perms}`, inline: true },
				{
					...
					value: `\`${
						command.clientPermissons == undefined
							? 'VIEW_CHANNEL'
							: command.clientPermissons
					}\`, \`SEND_MESSAGES\` `,
					inline: true,
				},
				{
					...
					value: `${
						new Date(
							fs.statSync(
								`/root/COOL-BOI-BOT/commands/${
									command.category
								}/${command.name.toLowerCase()}.js`,
							).mtime,
			...
			.setDescription(
				'[] arguments mean required, and () arguments mean optional. If theres none it means the there are no arguments nedded to run the command',
			)
